...
\begin{proof}
    ...
    Далее, пусть $E$ "--- произвольное измеримое множество. 
	 ...
	 По аксиоме выбора, $\exists G_{x,m} : G_{x,m+1} \subset G_{x,m}, E_x = \bigcup\limits_{m} G_{x,m}$.
	 Первый пункт выполняется, так как $E_x = \bigcup\limits_{m} G_{x,m} $ "--- не более чем счётное пересечение измеримых множеств. 
	 Также понятно, что $\lambda G_{x,m} \rightarrow \lambda E_x$. Следовательно, по теореме о поточечном пределе измеримых функций $\lambda E_x$ "--- измеримая, почти всюду конечная функция. \todo Пользуясь теоремой Лебега о предельном переходе под знаком интеграла получаем требуемое.
\end{proof}
